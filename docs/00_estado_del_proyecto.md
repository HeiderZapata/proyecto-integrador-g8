...
- **Yeison — orquestación + datos + A/B + documento.** (1) Correr la **auditoría de la capa de datos** (Medallion + EDA) con Claude Code en la rama `feat/auditoria-datos` (prompt ya preparado). (2) Con los hallazgos, **actualizar §13/§17** si algo cambia. (3) Armar los **planes de empalme** para Sara y Kelly. Luego: avanzar el diseño del **A/B test** y el documento.
...
| **vie 5 (hoy)** | ⏳ | **Auditoría capa de datos → empalmes con Sara/Kelly → arranque de modelo y tablero**; Heider: Ilustración 2 + `readStream`/MLflow |
