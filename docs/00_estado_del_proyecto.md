...
- **Yeison — orquestación + datos + A/B + documento.** (1) ✅ **Auditoría de la capa de datos HECHA** (Medallion + EDA, Claude Code, rama `feat/auditoria-datos`): **capa sin bloqueantes**, 9 checks en verde — reporte en `auditoria_capa_datos_2026-06-05.md`. (2) §13/§17 actualizados (cross-ref `agg_funnel_global`); pendientes menores cerrados (`_tmp_eda_units` borrado). (3) Armar los **planes de empalme** para Sara y Kelly (en el chat de Claude). Luego: avanzar el diseño del **A/B test** y el documento.
...
- **Heider — ingeniería.** Llave Kaggle **expirada ✅**. Aplicar los **3 ajustes de la Ilustración 2** (§2.3). Track paralelo (§2.3.1 paso 6): Bronze → **`readStream`** con checkpoint, **Spark SQL**, **MLflow setup**. **No tocar la Gold** (está congelada).
...
2. **MLflow:** params, métricas (**PR-AUC**, **Brier**/calibración), el **modelo calibrado** y la *signature*.
