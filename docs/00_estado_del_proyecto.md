...
**Última actualización:** lunes 9 de junio de 2026
...
>
> La secuencia §2.3.1 (tratamiento de datos) **se cerró**: **Gold de 22 columnas CONGELADA** (§13); EDA oficial re-fuenteado a **base limpia** (cuarentena transversal de la ventana corrupta 14–17 nov) con hallazgos y números **refrescados**; **particionamiento layer-aware** con evidencia medida (doc 02 §3); y **12 CSV agregados** para Power BI en `reports/data/` (verificados vs EDA). Esto **desbloquea a Sara (modelado) y Kelly (tablero)**. **Ingesta/Medallion SELLADO (7-jun):** Bronze por `readStream`/Auto Loader (Kappa), `user_id` determinista, cuarentena **14–17 nov** (ver §13 y §17 callout 7-jun).
...
> - **Cerrado tras el modelado (7-jun):** decisión de **split** train/test → **Opción C** (train oct + nov ≤ 23 / test 24–30 nov; ver callout de modelado y §17.3). **Aún abierto:** flag `sin_navegacion_previa` (§13, a confirmar con Sara) y los ajustes de la **Ilustración 2** de Heider (§2.3).
...
**Propensión** (`02_modelado_propension`): Dummy 0.056 → logística 0.0966 → comparación de familias RF/XGB/LGBM (empatadas dentro del ruido, 0.116–0.119; LightGBM por menor overfitting, brecha train−CV 0.018) → LightGBM+Optuna calibrado: CV 0.1235, **test PR-AUC 0.1172** (ex-BF **0.1115** = régimen normal), Brier 0.0515. Sin fuga (test≈CV). **Umbral operativo F1-óptimo: 0.092** → marca 15% de sesiones, lift ~2.2× (precision 12.1% vs base 5.6%). **Importancia por permutación** contrastada con la built-in de LightGBM: la built-in (split count) subestima `electronics_view_share` (0.042 vs 0.203 en permutación) por baja cardinalidad, lo que evidencia su sesgo y justifica la permutación. Features top: `max_price_viewed` (+0.038), `electronics_view_share` (+0.030), `avg_price_viewed` (+0.024) → coincide con EDA y clustering.
...
> **Actualización 9-jun (revisión Yeison + Claude Code):** (1) **Variabilidad reportada** — el modelo final da **PR-AUC CV 0.1235 ± 0.0014** (5 folds): no es un máximo aislado (R4). (2) **Calibración VERIFICADA en Databricks** — al recargar el modelo, el `06` reproduce **PR-AUC 0.1172 · Brier 0.0515 · prob media 0.059 ≈ tasa base**, tras corregir un **bug de orden de features** en el scoring (las 17 columnas iban permutadas → ranking revuelto; el Contrato 2 previo tenía probabilidades incorrectas y se **regenera** con el orden correcto). (3) Markdown **14–17** alineado · **R1** corregido (anti-fuga por features intra-sesión, no por disjunción de usuarios) · target A/B = **C3**. **(Cierre 9-jun pm):** Contrato 2 **regenerado y verificado** (vg prob_calibrada 0.060 ≈ tasa base, 19.71M filas en Delta tras corregir el orden de features); **informe consolidado** en ntrega_final/ (autores, enlace del tablero, 11 referencias APA, R4, validación del scoring) con TOC refrescado; **main conciliado y pusheado** (todas las ramas integradas: modelo/viz/pipeline/ab-test/doc/auditoría). Frentes de modelado, datos, tablero e informe **cerrados**. **PPT de defensa generada (9-jun):** 18 slides + notas del orador en `entrega_final/PPT_Defensa_PI_Grupo8.pptx` (reproducible con `entrega_final/build_pptx.py`) + guion slide-por-slide en `docs/guion_ppt_defensa.md`; el prompt de la PPT (`docs/prompt_ppt_defensa.md`) quedó **verificado y corregido** contra el repo (cifras + cobertura de rúbrica) y se le aplicó **auditoría adversarial multi-agente**. Resta **ensayo de defensa**, el **guion-por-expositor**, y confirmar 2 pendientes marcados en la propia PPT (jurado de Grandes Datos · si el tablero ya muestra el clustering C3).
...
> **Falta 1 día para la entrega (lun 8) y 2 para la exposición (mar 9).** La **capa de datos** está cerrada (Gold congelada, pipeline sellado, EDA oficial). **Modelo y tablero siguen en avance —aún no son finales—:** Sara está terminando de configurar, correr y comparar los modelos, y Kelly está cerrando el tablero con los ajustes sugeridos hoy y los nuevos CSV. Lo que queda es **llevar todo a producto**: modelo final, tablero final, informe y PPTX. El plan completo día a día está en **§15**; los frentes en **§11**; los contratos en **§13**.

## Hoy (dom 7, cierre) y mañana (lun 8) — foco: los tres productos

La **capa de datos** está hecha, pero **modelo y tablero todavía no están finales** — son los dos frentes vivos. Lo que viene: **Kelly termina el tablero** (ajustes sugeridos hoy + nuevos CSV + scores del modelo) y **Sara termina de configurar, correr y comparar los modelos** y cierra el scoring/Contrato 2. En paralelo arrancan los productos de entrega: **informe consolidado** y **PPTX**.

- **Sara — cerrar el frente de modelado (en avance, no final).** Propensión y clustering tienen resultados **preliminares** (ver callout de modelado); falta **terminar de configurar, correr y comparar** los modelos y, sobre el elegido, cerrar `06_scoring_mlflow_databricks`: **MLflow** (params, PR-AUC/Brier, modelo calibrado, signature) + **scoring batch** + **Contrato 2** (`user_session`, prob. calibrada, segmento) materializado en Delta. El Contrato 2 **desbloquea el tablero de Kelly y el segmento del A/B de Yeison** → **prioridad máxima**.
- **Kelly — visualización.** ✅ **Tablero Power BI v3 (9-jun) — 7 páginas.** (1) **Inicio:** portada minimalista con panel de navegación lateral. (2) **El problema:** embudo de conversión + 4 KPIs en rojo. (3) **Análisis por categoría:** 4 barras horizontales con semáforo verde/rojo/gris + filtro Top N (5/10/15). (4) **Detalle Electronics:** 2 KPIs + barras Top N marcas + scatter ticket vs abandono + filtro Top N marcas (5/10/20). (5) **Análisis de cliente:** donut tipología visitante + Sankey segmentos comprador (verde claro/verde oscuro). (6) **Contexto temporal:** combo barras grises + línea verde por hora (00:00–23:00). (7) **Cierre:** Pregunta de Oro + 3 tarjetas de respuesta (Electronics en rojo, comprador único y franja horaria en verde). Paleta finalizada: gris oscuro `#2D2D2D` estructural, verde `#3B6D11` oportunidad, rojo `#A32D2D` pérdida, gris `#BBBBBB` contexto; panel de navegación lateral en todas las páginas. **Entregables en el repo:** `.pbix` en `reports/powerbi/`, sección de visualización en `docs/`, capturas de las 7 páginas, paleta HTML. **Pendiente:** integrar resultados del modelo de clustering de Sara en página Análisis de cliente.
- **Yeison — A/B + documento + orquestación.** (1) Cerrar el **diseño del A/B test** sobre el segmento de mayor intención (C3 «electrónica precio medio» del clustering, el de mayor conversión). (2) Liderar el **documento consolidado del PI**, integrando los resultados de los cuatro frentes. (3) Coordinar con Sara el cierre del **Contrato 2** (corte exacto del split / posible híbrido C+A).
- **Heider — arquitectura + diagrama.** Aplicar los **3 ajustes de la Ilustración 2** (§2.3) —ya **desbloqueados todos**: el split quedó cerrado en **Opción C**— + **verificación final** del diagrama end-to-end (ingesta `readStream` → Medallion → consumo BI) contra la capa sellada. **No tocar la Gold** (congelada).
- **Equipo — defensa.** ✅ **PPTX generada** (18 slides + notas del orador, `entrega_final/PPT_Defensa_PI_Grupo8.pptx`; guion `docs/guion_ppt_defensa.md`). Resta **ensayo de defensa** filtrando en vivo + **Q&A por profesor** (insumo: `08_feedback_exposiciones_pregrado.md` §5) + **guion-por-expositor**.
...
| Tablero Power BI ejecutivo | Kelly | **v3 Hecho (9-jun)** | 7 páginas: Inicio · El problema · Análisis por categoría · Detalle Electronics · Análisis de cliente · Contexto temporal · Cierre. Paleta gris `#2D2D2D` / verde `#3B6D11` / rojo `#A32D2D` finalizada; navegación lateral. `.pbix` en `reports/powerbi/`, capturas y sección de viz en `docs/`. **Pendiente:** integrar clustering de Sara (Análisis de cliente) |
| Diseño del A/B test | Yeison | **Diseño redactado** | **Diseño completo en el informe §3.5** (en `main`): hipótesis, targeting con el modelo sobre **C3 «electrónica gama media»**, aleatorización por visitante, métricas + guardarraíl de margen, tamaño de muestra/MDE/potencia y duración. Cierre del alcance (responde al revisor: uplift→A/B) |
...
| Presentación (PPTX) | Equipo | **v1 generada (9-jun)** | `entrega_final/PPT_Defensa_PI_Grupo8.pptx` — 18 slides, paleta del tablero, notas del orador; guion `docs/guion_ppt_defensa.md`; script reproducible `build_pptx.py`. Cifras verificadas + auditoría adversarial. **Pendiente:** ensayo + guion-por-expositor + 2 confirmaciones (jurado Grandes Datos · clustering C3 en tablero) |
...
> 3. **Split train/test**. ✅ DESBLOQUEADO — el split se **cerró en Opción C** (train = oct + nov ≤ 23 / test = 24–30 nov; ver callout de modelado y EDA §6.1·bis). El diagrama fijaba "Train Octubre / Test Noviembre". → Rotular con el corte de la **Opción C**, o genérico "Split temporal (corte por fecha)" si se prefiere no fijar el mes en el diagrama.
>
> 4. **⏳ TAREA PENDIENTE (7-jun) — verificar que la Ilustración 2 refleje la versión FINAL de la capa (tras sellar ingesta/medallion).** Hubo cambios que el diagrama debe recoger: **(a) Bronze por Auto Loader (`cloudFiles`) + `Trigger.AvailableNow` + checkpoint = replay de streaming REAL (Kappa), ya NO batch** (el código dejó de ser batch); **(b) cuarentena 14–17 nov** (antes 15–17); **(c) `user_id` determinista en la Gold**; **(d) la capa de CONSUMO**: Gold agregada → **12 CSV** → Power BI. → **Contrastar el diagrama de Heider end-to-end (ingesta `readStream` → Medallion → consumo BI) contra estos ajustes antes de la entrega.** *(Hacerlo cuando la corrida de sellado esté verificada.)*
>
> *No se editó el diagrama de Heider (es su deliverable). Los **tres ajustes ya están desbloqueados** (el split cerró en Opción C); Heider puede aplicarlos todos. La cuarentena 14–17 nov puede mencionarse como nota de calidad en el diagrama si se quiere.*
...
| 5 | **Pasada 2 sobre la Gold** (según el EDA): correlación/multicolinealidad, variables para ML, métricas/tablas agregadas para el tablero, **calidad de datos** | Heider/Yeison | **Casi (§13 ✅)** | Cierra "Gold completa y robusta". ✅ **Hecho (4-jun):** calidad (cuarentena 14–17 nov + categóricas/precio/bots auditados), **8 features nuevas (22 cols)**, multicolinealidad resuelta (`revisit_intensity`), particionamiento layer-aware, **contrato §13 CONGELADO**, EDA re-fuenteado a base limpia con hallazgos refrescados. **Pendiente menor cerrado (5-jun):** `_tmp_eda_units` borrado. *(Block 3 — **12 CSV agregados** en `reports/data/` (funnel global/categoría/embudo, revenue-en-juego, marca-electronics, segmentos, tipología, métricas diarias + día×categoría + día×hora, hora×día-semana, marca-electronics-diaria), verificados vs EDA — **Hecho (7-jun)**.)* **Auditoría 5-jun: capa sin bloqueantes** (`auditoria_capa_datos_2026-06-05.md`). |
| 6 | **Track paralelo/posterior** (no gatea la correctitud de la Gold): Bronze→`readStream` (Kappa) · Spark SQL · MLflow tracking (✅ **verificado operativo 5-jun**) · scoring batch | Heider (readStream/SQL) · Heider/Sara (MLflow/scoring) | Parcial→✅ | **Bronze→`readStream` ✅ IMPLEMENTADO y verificado (7-jun): Auto Loader (`cloudFiles`) + `Trigger.AvailableNow` + checkpoint; 109.95M filas sin duplicar** (doc 02 §4). **MLflow tracking VERIFICADO operativo** (smoke test 5-jun en serverless; requiere `set_registry_uri("databricks-uc")` antes de `set_experiment` — gotcha Free Edition, doc 02 §4); el **logging del modelo** (params, PR-AUC/Brier, calibrado, signature) lo implementa Sara (§18.4). **Scoring batch es de los últimos pasos: requiere modelo entrenado** |

> **Nota de congelación (4-jun · ACTUALIZADA):** la pasada 2 (paso 5) **se ejecutó hoy** —calidad, cuarentena 14–17 nov, features enriquecidas (22 cols), multicolinealidad, particionamiento layer-aware— y el **esquema Gold quedó CONGELADO (§13)**. El EDA oficial se re-fuenteó a la **base limpia** (cuarentena transversal) y sus hallazgos/números se refrescaron. La regla "aditivo, nunca renombra ni elimina" sigue como red de seguridad. *Pendiente menor cerrado (5-jun): `_tmp_eda_units` borrado. Block 3 (**12 CSV**) ya en `reports/data/`.* **La auditoría del 5-jun (Medallion + EDA) confirmó la capa sin bloqueantes** (`auditoria_capa_datos_2026-06-05.md`; única recomendación opcional: `user_id` determinista, no bloqueante). La **decisión de split** queda **abierta con Sara** (§6.1·bis).
...
**Estado:** Fase 3 cerrada en la práctica —los cinco puntos cumplidos salvo el **Contrato 2** (salida del modelo), que lo cierra Sara con el scoring (§13)—. El proyecto está de lleno en **Fase 4**: pipeline sellado, EDA oficial, modelado avanzado y tablero v1; resta materializar los **productos** (informe, PPTX, tablero v2).
...
- **Volumen:** Oct (5.5 GB) + Nov (9 GB); 109.95M eventos → ~69.6M unidades full / **~56.76M en base limpia** (sin 14–17 nov; ~87.6M eventos limpios).
- **Métricas (base limpia · EDA oficial, cuarentena 14–17 nov · refrescado 7-jun):** cart **3.86%**, **conversión 2.27%**, **cierre 58.81%**, **abandono 41.19%** (unidad = producto-en-sesión). *Full-data (con la ventana corrupta): abandono 51.7% / cierre 48.3% — el 15-nov inflaba el abandono +8.6pp; ver §7/§17.*
...
- **Streaming liviano — ✅ IMPLEMENTADO (7-jun):** Bronze convertido a `readStream` con **Auto Loader (`cloudFiles`) + `Trigger.AvailableNow` + checkpoint** (replay Kappa, no en bucle; verificado 109.95M sin duplicar). Cubre la Unidad 4 y da sustancia a la narrativa Kappa. Nada de broker/productor/AWS.
...
## 15. Plan con fechas y dueños (mié 3 → mar 9) — AJUSTADO el 7-jun

Reanclado al cierre del dom 7: el **núcleo técnico ya está** (pipeline sellado, EDA oficial, modelado avanzado, tablero v1). Respeta la ruta crítica (§3) y la regla de cuota (no correr cargas pesadas la víspera, doc 02 §4). **El foco de hoy (dom 7, cierre) y mañana (lun 8) es de entrega: cerrar el scoring/Contrato 2 y materializar los tres productos —informe, PPTX y tablero v2—.** Sara y Kelly no esperan datos; el único pendiente técnico que alimenta producto es el Contrato 2 (scoring) → tablero v2 y segmento del A/B.
...
| **dom 7** (hoy) | 🔄 Sellado + modelado + tablero v1 | **Pipeline sellado** (`readStream`/Kappa, `user_id` determinista, cuarentena 14–17) + **12 CSV BI** + **EDA refrescado** + **modelado** (split C, propensión PR-AUC test 0.118, clustering k=4 — *cifras preliminares de ese día; el final fue test **0.1172** y **k=5**, ver callout de modelado*) + **tablero v1 publicado** (Kelly). Cierre nocturno. |
...
> **Nota de cronograma (7-jun):** el núcleo técnico está cerrado y la Gold congelada, así que el camino crítico de mañana es **producto, no datos**. El único pendiente técnico es el **Contrato 2** (scoring de Sara), que desbloquea el tablero v2 (Kelly) y el segmento del A/B (Yeison) → atacarlo primero. Regla de cuota: el **scoring batch** es la única corrida pesada que queda; conviene dejarla lista cuanto antes y, en la víspera de la expo (lun 8), no encadenar más cargas pesadas de datos.
...
> **🔧 Actualización 7-jun — ventana de cuarentena ampliada a 14–17 nov (antes 15–17).** El criterio original definía la ventana **solo por la etiqueta `purchase` rota** (15-nov = 0 compras, 16–17 volcado del backlog) → por eso el **14-nov se escapó**: tiene compras normales (22k) pero un **volcado de eventos** (2.86M views / 165k carts, *por encima del propio Black Friday*; conversión a la mitad, 0.77%). **Criterio nuevo: etiqueta rota _o_ volumen de eventos anómalo → 14–17 nov.** El **13-nov NO** entra (volumen normal: views 1.17× / carts 1.00×; su tasa algo baja es el régimen de inicios de noviembre). Impacto confirmado (corrida 7-jun): la conversión **subió** (2.24→2.27%) y el abandono **bajó** (43.1→41.19%). **✅ Refrescado (7-jun) con la cuarentena 14–17** — §17.1, §6, §13 y los **12 CSV** ya reflejan la base sin el 14. Titular nuevo: **conv 2.27% · abandono 41.19% · cierre 58.81% · 56.76M unidades · 903k carritos · $250.4M en juego**.

### 17.1 Hallazgos de negocio (base limpia · cuarentena 14–17 nov · 87.6M eventos / 56.76M unidades · refrescado 7-jun)
- **Funnel por unidad (base LIMPIA):** cart **3.86%**, conv **2.27%**, cierre **58.81%**, **abandono 41.19%** (56.76M unidades; 903k carritos abandonados). *(La ventana corrupta inflaba el abandono a 51.7% y deprimía el cierre a 48.3%; full-data en §7.)* El negocio es **electrónica**: conv **3.58%**, mayor volumen (20.03M unidades), y el mayor pool de carritos abandonados ≈ **$187.1M en juego** (de $250.4M totales; full-data inflaba electronics a $349.5M). **Concentración de revenue:** electronics **76.9%**, top-3 **87.3%**; el 50% del revenue lo hacen **50 productos** (0.1%). *(Revenue se basa en compras, casi sin cambio al excluir el 14 —cuyo defecto estaba en views/carts, no en compras—; los CSV por-categoría `agg_funnel_categoria`/`agg_revenue_en_juego` **excluyen `Unknown`** (~32%); el funnel y totales **globales** del titular —incl. `Unknown`, 56.76M unidades, 903k carritos, $250.4M— viven en `agg_funnel_global.csv`. Detalle en `reports/data/README.md`.)*
- **Dos palancas (síntesis §4.9):** (A) recuperar carritos de alta intención que no cierran (electronics; **Samsung+Apple = 68.6%** de carritos electronics, abandono ~39–40%); (B) retener al núcleo recurrente (**35.7%** de compradores = **73.7%** del revenue; ticket recurrente **$1,460** vs one-time **$289**; 2ª compra mediana **2.9 días** —37% ≤1 día—, **84.4%** a la misma categoría → cross-sell 15.6%).
