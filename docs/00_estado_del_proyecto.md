...
>
> La secuencia §2.3.1 (tratamiento de datos) **se cerró**: **Gold de 22 columnas CONGELADA** (§13); EDA oficial re-fuenteado a **base limpia** (cuarentena transversal de la ventana corrupta 14–17 nov) con hallazgos y números **refrescados**; **particionamiento layer-aware** con evidencia medida (doc 02 §3); y **12 CSV agregados** para Power BI en `reports/data/` (verificados vs EDA). Esto **desbloquea a Sara (modelado) y Kelly (tablero)**. **Ingesta/Medallion SELLADO (8-jun):** Bronze por `readStream`/Auto Loader (Kappa), `user_id` determinista, cuarentena **14–17 nov** (ver §13 y §17 callout 8-jun).
...
| Tablero Power BI ejecutivo | Kelly | **v1 Hecho (7-jun) · v2 en curso** | 4 páginas publicadas en Power BI Service, sobre la Gold **agregada** (**12 CSV** en `reports/data/`). Insumos nuevos (8-jun): toggle día↔hora, embudo, heatmap hora×día, día×categoría, marca-electronics-diaria. Pendiente: conectar scores de Sara (Contrato 2) + matriz de oportunidad |
...
- **Métricas (base limpia · EDA oficial, cuarentena 14–17 nov · refrescado 8-jun):** cart **3.86%**, **conversión 2.27%**, **cierre 58.81%**, **abandono 41.19%** (unidad = producto-en-sesión). *Full-data (con la ventana corrupta): abandono 51.7% / cierre 48.3% — el 15-nov inflaba el abandono +8.6pp; ver §7/§17.*
